...
public class membershipController {
    ...
    @GetMapping("/api/test/user/allMembership")
    private List<membership> getAll(){
        return membershipService.getAll();
    }

    @CrossOrigin(origins = "http://localhost:8080")
    @PostMapping("/api/test/user/createMembership")
    ...
    @CrossOrigin(origins = "http://localhost:8080")
    @DeleteMapping("/api/test/user/disableMembership/{membership_name}")
    ...
    @CrossOrigin(origins = "http://localhost:8080")
    @GetMapping("/api/test/user/allPOI")
    private List<String> allPOI(){
        return membershipService.getAllPOI();
    }
