...
public class corporate_passService {
    ...
    public String createPass(corporate_passRequestBody jsonBody) {
        ...
        List<corporate_pass> all_corporate = new ArrayList<>();
        corporate_passRepo.findAll().forEach(item -> all_corporate.add(item));
        int counter = 0;
        for (int i = 0; i < all_corporate.size(); i++) {
            if (all_corporate.get(i).getCardNumber().replaceAll("\\s", "").equals(cardNumber.replaceAll("\\s", ""))) {
                counter += 1;
            }
        }

        if (counter == 1) {
            return "1Card Number: " + cardNumber + " already exists!";
        } else {
            // Get the membership object that matches the place of interest passed from the
            // json request body
            ...
            // Loop through the all_membership list and fetch the membership id that matches
            // the place of interest passed from the
            // json body
            for (int i = 0; i < all_membership.size(); i++) {
                if (all_membership.get(i).getPOI().replaceAll("\\s", "").toLowerCase()
                        .equals(place_of_interest.replaceAll("\\s", "").toLowerCase())) {
            ...
            return "Successfully created a new corporate pass with Card Number: " + cardNumber;
        }
