...
public class loanService {
    ...
    public String createLoan(loanRequestBody jsonBody){
        ...
        if(num_pass > max_pass_per_loan){
            return "aExceeded the limit of " + max_pass_per_loan + " passes per loan for " + "\"" + placeInterest + "\"";
        }else{
            /*
            2nd Logic Check:
            ...
            */
            ...
            if(loan_match_employee_id.size() == 0){
                ...
                for( LocalDateTime date_created : keys ){
                    ...
                    if( year.equals(year_from_db) && month.equals(month_from_db) && (loan_row.getStatus() == 0 || loan_row.getStatus() == 2)){
            ...
            if(month_current_loan == max_loan_per_month){
                return "bExceeded the limit of " + max_loan_per_month + " loans per month";
            }else{
                /*
                3rd Logic Check:
                ...
                */
                ...
                if(available_pass < num_pass){
                    return "cThere are insufficient passes to be loaned for indicated place of interest: " + "\"" + placeInterest + "\"";
                }else{
                    /*
                    ...
                    */
                    String type = "";
                    String number = "";
                    LocalDate time = LocalDate.parse("2020-01-08");
                    ...
                    for(int i = 0 ; i < num_pass ; i++){
                        ...
                        String cardType = corporate_pass.getCardType();
                        type = cardType;
                        number += cardNumber + " ";
                        int status = 0;
                        int min = 0;
                    ...
                    if (type.equals("Physical")) {
                        String plaintextMail = "Dear %s, %n%nWe are pleased to inform that your booking to %s is confirmed as follows:%n%nDate of Visit : %ta %td %tB %tY (1 day only)%nMembership ID : %s%n%nFor any change in visit date, you are required to cancel your booking (at least 1 day before) and to submit a new booking in the system. Attached is the authorisation letter to %s. Please check that the details are accurate. %n%nPlease take note of the following on the day of your visit to %s:%n●	Present this email confirmation to the General Office to collect the membership card(s).%n.●	Collect the membership card(s) from the General Office one day before your visit date and return the membership card(s) by 9am the next working day after your visit. %n●	Present the membership card(s), the authorisation letter and your staff pass at the entrance of %s.%n●	Entry is strictly based on the membership card(s) and the authorisation letter. %n●	Your presence is required on the day of visit. Entry will be denied without staff’s presence. %n●	Your booking is non-transferable.  %n ●	Visit date is strictly based on the date stated in this email and the authorisation letter. ●	Staff found abusing the membership(s) will be subject to disciplinary actions. %n%nEnjoy your visit to %s!%n%nWarm regards%nHR Department";

                        String body = String.format(plaintextMail, userName, placeInterest, time, time, time, time, number , placeInterest, placeInterest, placeInterest, placeInterest); 
                        try { 
                            senderService.sendMessageWithAttachment(email, "Booking Confirmation", body, "authorisationLetter.pdf"); 
                        } catch (Throwable e) { 
                            System.out.println(e.getMessage()); 
                        }
                    }
    ...
    public List<loan> retrieveLoanDate(String date){
        
        List<loan> return_list = new ArrayList<>();
        
        List<loan> all_loan = new ArrayList<>();
        loanRepo.findAll().forEach(loan -> all_loan.add(loan));

        for(int i = 0 ; i < all_loan.size() ; i++){
            loan loan = all_loan.get(i);
            int status = loan.getStatus();
            Date date_Visit = loan.getDateVisit();
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
            String date_visit_format = formatter.format(date_Visit);
            if(date_visit_format.equals(date) && status != 1 && status != 3){
                return_list.add(loan);
            }
    ...
    //e.g localhost:8080/updateLoanRecord/1 => loanID value of 1
   
    public String updateRecord(int id , int state) {
        //loan ID , status and email
        loan loan = loanRepo.findById(id).get();
        Integer loanID = id;
        Integer status = state;
        String email = loan.getUser().getEmail();
        Date dateVisit = loan.getDateVisit();
        // int employeeID = obj.getEmployee_ID();

        //    status: 0 => active, 1 => cancelled, 2 => collected, 3 => returned
        if (!loanRepo.findById(loanID).isPresent()) {
            return "aRecord does not exist!";
        }
        ...
        if (status == 1) {
            ...
            if (diffInDays >= 1) {
                ...
                loanRepo.setLoanStatus(status, loanID);

                return "bLoan Pass was cancelled by user successfully.";
            }

            return "cThe intended visit date is today! Cancellation has to take place latest 1 day before intended visit date";
        }
        ...
        if (status == 2) {
            ...
            // String uri = "http://localhost:8080/getEmployee/" + employeeID;
            // RestTemplate restTemplate = new RestTemplate();
            // String result = restTemplate.getForObject(uri, String.class);

            senderService.sendSimpleMessage(email, "Confirmation", "You have collected your loan pass");

            return "dLoan Pass collected by borrower.";
        }

        if (status == 3) {
            loanRepo.setLoanStatus(status, loanID);

            return "eLoan Pass returned by borrower.";
        }

        return "Loan Pass is already active. Awaiting collection";
    }
    ...
    public List<loan> getLoanByE(int id){
        List<loan> all_loan = new ArrayList<>();
        loanRepo.findAll().forEach(item -> all_loan.add(item));

        List<loan> return_list = new ArrayList<>();

        for(int i = 0 ; i < all_loan.size() ; i++){
            if(all_loan.get(i).getUser().getEmployeeID() == id){
                return_list.add(all_loan.get(i));
            }
        }
